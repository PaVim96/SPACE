...
def evaluate_z_what(arguments, z_what, labels, image_refs_tuple, nb_samples, cfg):
    ...
    if arguments['edgecolors']:
        ...
        if wantsHover:
            highlightButton = CheckButtons(ax, ["POI"])
...
if __name__ == "__main__":
    ...
    if wantsHover:
        ...
        assert os.path.isdir(org_path), f'The directory {org_path} doesnt exist. It should contain the actual images on which z_what_train was trained on. Additionally, make sure that image_refs'
        image_refs = [ref for singleLst in image_refs for ref in singleLst]
        image_refs_tuple = (org_path, image_refs)
    else:
        image_refs_tuple = None
